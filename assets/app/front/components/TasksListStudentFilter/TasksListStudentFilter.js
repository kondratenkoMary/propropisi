...
import './TasksListStudentFilter.less';
...
const Selectors = {
    el: '.TasksListStudentFilter',
    filterText: '.TasksListStudentFilter-selectText',
    filterMenu: '.TasksListStudentFilter-selectMenu',
    filterMenuItem: '.TasksListStudentFilter-selectMenuItem',
    filterMenuCheckbox: '.TasksListStudentFilter-selectMenuItem input'
};
...
export default class TasksListStudentFilter extends Base {
    events() {
        return {
        ...
        if (data.length && data.length < 4) {
            _.each(data, (item) => {
                this.$(Selectors.filterText)
                    .append(
                        '<span class="TasksListStudentFilter-selectText--' + item.id + '">' + item.text + '</span>'
                    );
            });
        } else {
            this.$(Selectors.filterText)
    ...
    onApplyFilters(data) {
        app.vent.trigger('TasksListStudent-filter', data);
    }
};
